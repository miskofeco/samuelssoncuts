...
- Push delivery is best-effort and non-fatal. Notification rows are inserted before `after()` defers push delivery beyond the action/route response. Invalid subscriptions are deleted or marked failed. Push payloads are structured for the lock screen: a short action title plus a detail body built from structured fields only (client name for the barber, service, Slovak date, time) by `src/domain/push-copy.ts` and passed as `push` on the notification input. Without `push`, the in-app subject is the title. The free-text notification `body` (client notes, admin reasons) never reaches the push service.
