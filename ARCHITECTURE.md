...
- Google sign-in and sign-up are one flow: Supabase creates the auth user on the first consent. `OAuthButtons` takes `intent` (`login` on `/login`, `register` on `/register`); `signInWithOAuthAction` stores it in the short-lived, httpOnly `oauth_intent` cookie (path `/auth`) so `redirectTo` stays the exact allow-listed `/auth/callback`. After the code exchange the callback (`src/domain/oauth-landing.ts`) sends an account without a phone to `/complete-profile`; from the login page it adds `?notice=google_registered` (new account: "not registered yet, registration started") or `registration_incomplete`. If Supabase refuses the new user because sign-ups are disabled (`error_code=signup_disabled`), the callback redirects to `/register?error=oauth_not_registered` instead of calling it a cancellation. Other failures return to the page the flow started from, and the cookie is cleared on every outcome.
- Auth pages never render free text from the URL. Route handlers and redirecting actions pass short codes (`?error=oauth_failed`, `?notice=confirm_sent`) that `src/i18n/auth-notices.ts` resolves to localised copy; unknown codes render nothing. `/auth/callback` maps a plain Google `access_denied` to `oauth_cancelled`, and failed recovery links (callback, confirm or a missing session on `/auth/update-password`) go to `/reset-password?error=reset_link_invalid`.
...
- The `handle_new_user` trigger on `auth.users` creates every profile (email and Google sign-ups) and calls `normalize_phone()`, so a broken helper blocks all registrations with Supabase's opaque "Database error saving new user". Migration `0053` fixes the `0043` regex (`'[[:space:]\\-().]'` was an invalid character range under standard-conforming strings) and makes a concurrently taken phone create a phone-less profile that `/complete-profile` then fills, instead of aborting sign-up; `tests/database/0053_fix_normalize_phone_regex.sql` exercises email, Google and duplicate-phone sign-ups. Keep phone regexes escape-free (hyphen last in the bracket).
- Auth actions classify Supabase errors with `authErrorKind` (`src/domain/auth-errors.ts`) by `code`/status: email taken or invalid, weak password, rate limited, sign-ups disabled, auth-email hook delivery failure, bad credentials, unconfirmed email, same password, banned. Each maps to localized copy; only unknown failures (including database errors) are reported as faults. `reportError` serialises plain error objects such as `PostgrestError` (message, code, details, hint) rather than `[object Object]`.
...
Migration `0043` validates proposal ownership against `auth.uid()` inside the authenticated RPC, enforces the booking window/hours/blocks on acceptance, restricts Web Push endpoints to supported HTTPS providers, caps stored subscriptions per account, and reconciles selected function grants/search paths. Migration `0044` adds service-role-only transactional account deletion, manual booking and proposal replacement RPCs, stores appointment price/note snapshots, and clears an obsolete selected proposal when a confirmed request reopens for rescheduling. Migration `0045` serializes appointment, block and business-hour writes by barber with a transaction advisory lock, then checks the opposing records inside security-definer triggers; it protects against a new booking racing a closure. Migration `0046` adds the service-role-only `admin_create_booking_priced` RPC, which saves a manually agreed price on the appointment and backing request within the same transaction. Migration `0048` adds `services.sunday_price_cents` and makes the appointment price-snapshot fallback use it for appointments starting on a `Europe/Bratislava` Sunday. Migration `0050` adds `appointments.availability_override` and an explicit `p_allow_unavailable` parameter to `admin_create_booking`/`admin_create_booking_priced` (old signatures dropped, service-role only): with it, the RPC waives only the opening-hours and blocked-time checks and flags the appointment when the slot really is unavailable. The appointment write guard skips hours/blocks for flagged rows, clears the flag whenever the appointment is moved unless the writer sets it again, and the hours/block write guards ignore flagged appointments. Past, same-day and confirmed-overlap checks always apply. `tests/database/0050_admin_availability_override.sql` exercises this against a disposable PostgreSQL. Apply `0043`–`0046` in order before deploying code that calls their RPCs or reads the new appointment columns.
...
- `services`: barber services, duration, regular (Monday–Saturday) `price_cents`, separate `sunday_price_cents` (migration `0048`, backfilled from the regular price), active flag, optional image. Catalog prices are whole euros: migration `0049` rounded existing prices (halves up) and adds `% 100 = 0` checks, and the service actions validate `multipleOf(100)`.
- `business_hours`: weekly open/closed windows.
- `blocked_times`: date or time ranges the barber cannot take bookings, each with a required short `reason` (2–40 characters; `blockDateAction` validates it and migration `0051` adds the check as `NOT VALID`; `0052` backfills missing/too-short reasons with "Voľno" and then validates it). `loadBlockedDays` converts rows with `blockedRangeFromRow` (instant comparison, so whole-day blocks show dates without 00:00 times) and returns reason-free `intervals` for client payloads plus `labeledIntervals` that only the admin calendar loader exposes; the calendar shows reasons on blocked day headers, blocked bands, month cells, the day view and phone cards, and the availability list leads with the reason.
...
- `schedule.ts`: services defaults, slot generation, booking window rules, month/week helpers, business-hour checks, preferred slot/gap/VIP pricing helpers. `servicePriceForDate()` picks a service's Sunday or regular list price for a shop-local day; every client/admin price preview uses it as the base before surcharges. `priceCentsForSlot()` rounds every quote (base, gap, VIP) to whole euros, halves up; `parseEuroCents()` accepts whole-euro manual prices only (`createAdminBookingAction` also validates `multipleOf(100)`); `formatEuroAmount()` displays stored cents without decimals when whole, and keeps legacy fractional snapshots exact.
...
2. UI uses `clientSlotsForService`, `slotStatusFor`, and cent-precise pricing to display availability and quotes. Sunday follows the same booking rules as any other open day (hours, blocks, gap/VIP surcharges), but its base is the service's barber-set Sunday price; `quoteClientSlot`/`quoteAdminSlot` in `src/server/booking-pricing.ts` receive both prices and choose by `isSundayDate`. Whether Sunday is bookable at all is still controlled by business hours (closed by default). Every client slot starting at or after 17:00 uses the barber-configured VIP surcharge, even when it touches another booking; the same precedence applies to the server quote and its saved surcharge flag. Partial-day blocks remain UTC intervals until each candidate appointment is checked for overlap; only complete closures become blocked calendar days. Existing appointments use their stored start/end duration and booked service identity even if the public service catalog later changes.
...
- The admin calendar derives each day's unavailable time with `dayAvailability()` (`src/domain/schedule.ts`) from the barber's business hours and blocked-time intervals. Colors carry the source of unavailability: time closed by opening hours (outside a day's window, or a weekday switched off) is grey, and blocked-time ranges (partial or whole-day) are red, painted over grey where they overlap (`unavailableKindAt` prefers blocks). Hour gridlines stay visible above both. A closed weekday or a fully covered opening window is a day off in every view (grey "closed" or red "blocked" header, month cell and phone card). The day agenda shows the opening range and partial blocks, and every "+ Add" default starts at the first bookable minute. The add-booking modal names the real reason for an unbookable time (closed day, outside hours, blocked, overlap). The barber may still book closed or blocked time: every calendar add goes through `requestAdd`, which shows `UnavailableBookingConfirm` before the modal opens, and the modal offers the same confirmation for a closed/blocked date or time picked inside it. Only then does `adminSlotOptions({ allowUnavailable })` enable those starts and `createAdminBookingAction` pass `allowUnavailable` to `guardSlot` and the RPC. Client booking paths never set it.
